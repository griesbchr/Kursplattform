...
function getTeacherFile(id)
{
  ...
  try{
    ...
    throw new Error("Could not open Teacherfile for teacher with id " + id + " and file id + "+file_id+" due to error " + e.message)
  }  
...
function getAllContactStatus()
{
  ...
  while (file_iter.hasNext()) 
  {
    ...
    var student_sheet = SpreadsheetApp.openById(file_id).getSheets().filter(e => e.getName() === STUDENTSSHEETNAME)[0]
    //get all valid cols
    var id_values = student_sheet.getRange(STU_COLNAMEROW+1,STU_IDCOL,student_sheet.getLastRow()).getValues().flat().filter(String)
    ...
    if (last_row >= STU_COLNAMEROW+1) //no students in file
    {
      var contact_status = student_sheet.getRange(STU_COLNAMEROW+1,STU_CONTACTSTATUSCOL,last_row-STU_COLNAMEROW).getValues().flat()
      var course_status = student_sheet.getRange(STU_COLNAMEROW+1,STU_BILLINGSTATUS,last_row-STU_COLNAMEROW).getValues().flat()
      var student_fist_name = student_sheet.getRange(STU_COLNAMEROW+1,STU_FIRSTNAMECOL,last_row-STU_COLNAMEROW).getValues().flat()
      var student_last_name = student_sheet.getRange(STU_COLNAMEROW+1,STU_LASTNAMECOL,last_row-STU_COLNAMEROW).getValues().flat()
      id_values.forEach((key, i) => student_dict[key] = [contact_status[i], student_fist_name[i] + " " + student_last_name[i], teacher_id, teacher_name, course_status])
    }
...
{
  return "https://docs.google.com/spreadsheets/d/"+sheet.getParent().getId()+"/edit#gid="+sheet.getSheetId()+"&range="+row1+":"+row2
}
...
function appendCleanRows(sheet, n)
{
  ...
  sheet.insertRowsAfter(last_row, 2)
...
{
  range.setBorder(null, null, null, null, true, true, null, SpreadsheetApp.BorderStyle.SOLID)
