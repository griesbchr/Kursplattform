function addCourseToAttendancelist(course) 
{
  ...
  try {
    ...
    sheet.getRange(upper_row, ATT_FIRSTNAMECOL, 1, 2)
  ...
  sheet.getRange(upper_row, ATT_FIRSTNAMECOL, 2, 2)
  ...
  sheet.getRange(upper_row, ATT_LASTNAMECOL+1, 2,course["Vertrag_Einheiten"])
    .setBorder(true, true, true, true, true, true, null, SpreadsheetApp.BorderStyle.SOLID)
  sheet.getRange(upper_row, ATT_LASTNAMECOL+1, 2,MAXLESSONS)
  ...
  if ((ATT_NOCOURSELOCATIONHOURSCOL-ATT_LASTNAMECOL-course["Vertrag_Einheiten"]-1) > 0) //33 - 2 - 30/29/28/27 - 1 = 0/1/2/3
  {
    var protection = sheet.getRange(upper_row, ATT_LASTNAMECOL+course["Vertrag_Einheiten"]+1, 2, ATT_NOCOURSELOCATIONHOURSCOL-ATT_LASTNAMECOL-course["Vertrag_Einheiten"]-1).protect()
    protection.removeEditors(protection.getEditors())
              .addEditors(EDITORSMAILLIST)
              .setDescription("not allowed units")
  ...
  sheet.getRange(upper_row, ATT_ACTUALHOURSCOL, 2).merge()
    .setFormula("=COUNTA(C"+lower_row+":AF"+lower_row+')') //=COUNTA(D33:AG33)
  sheet.getRange(upper_row, ATT_NOCOURSELOCATIONHOURSCOL, 2, 3)    
  ...
  var expected_hours_less_than_max_lessons_rule = SpreadsheetApp.newDataValidation()
                        ...
                        .setHelpText("Anzahl der voraussichtlichen Kurseinheiten darf die Anzahl der im Kursvertrag festgelegten Kurseinheiten nicht überschreiten.")
                        .build()
