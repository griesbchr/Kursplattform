...
function createCourse(course)
{
  ...
  Logger.log("[TF]Added course " +  course["KursID"] + " of student " + course["SchuelerID"] + " to teacher " + course["LehrerID"])
}
...
function getCourseHours(course)
{
  var course_hours;
  //set contract lessons and room rent
  if (course["Kursmodus"] == ROOMRENTATCOURSELOCATIONNAME) // am Kursort
  {
    course_hours = 30-DATA.getRoomRent(course["Zweigstelle"])
  }else if (course["Kursmodus"] == ROOMRENTATSTUDENTNAME) // bei Schüler
  {
...
function changeStudentSheet(course)
{
  ...
  let student_sheet = SpreadsheetApp.openById(file_id).getSheets().filter(e => e.getName() === STUDENTSSHEETNAME)[0]
  ...
  var protection = student_sheet.getRange(row, STU_COURSEPARAMSTARTINGCOL, 1, STU_COURSEPARAMLENGTH)
                        .setFontColor(CANNOTCHANGECOLOR)
                        .protect()
protection.removeEditors(protection.getEditors())
          .addEditors(EDITORSMAILLIST)
          .setDescription(course["KursID"])
