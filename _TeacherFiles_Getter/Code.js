  ...
  for (var file_id of file_id_list) {
    try {
      ...
      for (var i = 0; i < data_values.length; i++) {
        ...
        if (id) {
          if (student_dict.hasOwnProperty(id))      //only add Instrument
          {
...
function getRoomBillingData()
{
  ...
  while(file_iter.hasNext())
  {
    ...
    var room_billing_units = teacherfile_ss.getSheetByName(FORMULASSHEETNAME).getRange(FORMULA_VALUEROW, FORMULA_ROOMBILLINGFACTORCOL).getValue()
    // ####DETAILS DATA#####
    var details_values = teabillingsheet.getRange(BILTEA_ROOMBILLINGDETAIL_FIRSTROW, BILTEA_ROOMBILLINGDETAIL_FIRSTCOL, BILTEA_ROOMBILLINGDETAIL_LASTROW - BILTEA_ROOMBILLINGDETAIL_FIRSTROW + 1, BILTEA_ROOMBILLINGDETAIL_LASTCOL - BILTEA_ROOMBILLINGDETAIL_FIRSTCOL + 1).getValues()
    ...
    // ####OVERVIEW DATA#####
    var overview_values = teabillingsheet.getRange(BILTEA_ROOMBILLINGOVERVIEW_FIRSTROW, BILTEA_ROOMBILLINGOVERVIEW_FIRSTCOL, BILTEA_ROOMBILLINGOVERVIEW_LASTROW - BILTEA_ROOMBILLINGOVERVIEW_FIRSTROW + 1, BILTEA_ROOMBILLINGOVERVIEW_LASTCOL - BILTEA_ROOMBILLINGOVERVIEW_FIRSTCOL + 1).getValues()
