function printFileIDs()
{
  var file_iter = DriveApp.getFolderById("1VOoVD7zNlcy6wSXDAK0vJIgom9q8qeFS").getFiles()
  var file_ids = []
  while (file_iter.hasNext())
  {
    var file = file_iter.next()
    file_ids.push(file.getId())
  }
  console.log(file_ids)
}

function testMergePDFs() {
  const targetFolderId = '1VOoVD7zNlcy6wSXDAK0vJIgom9q8qeFS'

  // IDs of the PDF files to merge
  fileIds =[ '1DoTWMeO5eZ22Gk-fx9qERhD4JmKu30GV',
  '1T0uzbNnPcw3VjouXlob0qTxTPphXM3R5',
  '1feT8zf133Em_0AkRuVsCuMVvphDUPf3a',
  '1E0riNNw8rdettkD3GVclUcsSp-UNFNoP',
  '1fP1ns23xnMHppE9b8G9_GQT5u2_DNhKj' ]; // Replace with your PDF file IDs

  var mergedFileName = "merged"
  const result = mergePdfs(fileIds, targetFolderId, mergedFileName)

  Logger.log('Merged PDF created successfully:');
  Logger.log('File ID: ' + result.fileId);
  Logger.log('File Name: ' + result.fileName);
  Logger.log('File URL: ' + result.fileUrl);
  Logger.log('Total files merged: ' + result.totalMergedFiles);
}
 ...
 */
function mergePdfs(file_ids, target_folder_id, name) {
 
  var directory = DriveApp.getFolderById(target_folder_id)
  
  var file_list = []
  for (var file_id of file_ids)
  {
    file_list.push(DriveApp.getFileById(file_id))
  }
  ...
  function getObject(objectAddress, objectByteOffsets, bytes) {
    ...
    var object = [];
    var byteIndex = 0;
    for (var offset in objectByteOffsets) {
