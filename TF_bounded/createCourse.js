function createCourseCallback() {
  ...
  if (rows.length > 15) 
  {
    SpreadsheetApp.getActiveSpreadsheet().toast("Es können maximal 15 Kurse gleichzeitig gestartet werden. Bitte wählen Sie weniger Kurse aus. Die verbleibenden Kurse können durch eine zweite Aktion gestartet werden.", null, 30)
    return 
  }
  ...
  for (var row of rows) {
    ...
    switch(course_type){
      ...
      if (course_number == 15 || course_number >= 70)
      {
        invalid_rows.push(row)
      }
      break
      ...
      case COURSETYPE_QV:
      if (course_number == 15 || course_number == 25 || course_number >= 80)
      {
        invalid_rows.push(row)
      }      
      break
  ...
  //messagebox results
  if (result == "OK") {
    SpreadsheetApp.getActiveSpreadsheet().toast("Verrechnungsprozess wird gestartet. Dies kann einige Minuten dauern. Bitte keinesfalls auf 'Abbrechen' drücken!",null,30)
    //uncheck rows
    for (let row of rows) {
      STUDENTSSHEET.getRange(row, STU_CHECKBOXCOL).uncheck()
    }
    ...
    for (let row of rows) {
      ...
      postCourseToTeacherFiles(course)
    }
  } else {
